...
**Bounded Context** is an explicitly defined scope of a particular model as bounded part of the software system, within which a model is kept as unified as possible. Larger systems usually have multiple contexts (orders, user management, shiping are examples of the separate contexts within an online retail system). Interaction between bounded contexts is organized via Integration Events in Spine.
